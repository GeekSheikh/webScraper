...
url = 'http://www.imdb.com/search/title?at=0&sort=num_votes,desc&start=1'
html = urllib2.urlopen(url).read()
soup = bs4.BeautifulSoup4(html)
soup.findAll('h3',{'class': 'lister-item-header'})
titles = []
for title in soup.findAll('h3',{'class': 'lister-item-header'}):
  titles.append(title.get('a'))
titles
...
def scrape_many(start=1,
                ...
                pages=10,
                second_delay=1,
                url_format="http://imdb.com/search/title?at=0&sort=num_votes,desc&start={start}"):
    """
...
movies = scrape_many(start=1, step=5, pages=2)
movies